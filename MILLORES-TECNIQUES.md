...
| Data | ID | Canvi | Qui |
|------|----|-------|-----|
| 2026-09-16 | dades | Desplegada l'edició CB 2026 a l'origen. Fins ara només existia a l'eix Florence: les targetes `CB158…CB218` i el `PAYLOAD` ja la referenciaven des del 2026-09-04, però `preguntes.json` de `cb-main` es va quedar al 2025 i `valida-dades.js` avisava que `cb-items.json` anava 59 preguntes endarrerit. Generats els 86 PNG amb `retalla_cb.py` (12+13 enunciats, 32+29 preguntes) i afegides 61 entrades a `preguntes.json` (4t ids 158–189, 2n 190–218), amb el sentit, el grau i la clau transcrits dels documents oficials d'especificacions. Omès l'ítem 12 de 2n (Verdader/Fals de dues parts) i, aquest cop sí, també la seva imatge. Els 8 ítems EiM de 4t (3, 4, 7, 8, 9, 10, 24, 26) repartits entre `espacial` i `mesura` seguint el que ja fixaven les targetes. `cb-items.json` regenerat: 59 blocs · 157 preguntes → **84 blocs · 218 preguntes**, anys 2022–2026. Verificació: els retalls nous reprodueixen píxel a píxel les 59 targetes publicades (error màxim 0,31/255, el de la palatització) i els 59 sentits del document oficial coincideixen amb els de les targetes sense cap excepció. `ARQUITECTURA.md` §3 al dia. **Pendent:** les 61 preguntes tenen les `pistes` buides, de manera que el mode pràctica mostra el requadre de retroacció en blanc quan l'alumne falla | — |
| 2026-09-07 | dades | Retirats del catàleg 33 materials i el valor d'`origin` que els agrupava. A `manifest.json` desapareixen les 33 entrades de `files[]` (22 dossiers + 11 solucions, tots d'ESO, tots PDF) amb els seus 33 `drive_id`, i el valor d'origen surt de `vocabulary.origin`, `vocabulary_labels` i `basic.origin`: 168 fitxes → 135, 13 orígens → 12, 136 etiquetes → 135. Comprovat que cap fitxa supervivent no en referencia cap (`solution_of` inclòs). Es queden al vocabulari, ara sense cap fitxa que els faci servir, el tipus `solucions` i tres activitats de 4t (`funcio-exponencial-logaritmica`, `geometria-analitica-pla`, `rectes-pla`): el vocabulari és la llista de valors permesos per a material futur, no un índex del que hi ha, i el catàleg ja arrossega altres valors sense ús (`reptes-mates`, `primaria`, els tipus d'examen). A la interfície surten atenuats amb `is-zero`, i cap no és a `basic`, així que en mode Bàsic no es veuen. `index.html`: `syncFromUrl()` ara descarta els valors de filtre que no siguin al vocabulari; sense això, un enllaç antic amb el valor retirat a `?origin=` l'hauria repintat al xip a través del fallback de `label()` (slug → «Text Capitalitzat») i, de propina, hauria retornat zero resultats sense dir per què. Els `?open=` que apuntin a una fitxa esborrada ja es gestionaven sols: `renderPreview()` tanca el panell i el menú de recents filtra pel manifest. `ARQUITECTURA.md` §2: posades al dia les xifres del contracte de `manifest.json`, que anaven endarrerides des de molt abans d'aquest canvi (deien 48 entrades i «version: 1»); ara `version` 4, 135 entrades, 135 etiquetes, 91 activitats, i la freqüència de camps i la distribució de format recomptades. Afegit a «Com funciona el vocabulari» per què hi ha valors sense cap fitxa i per què no s'esborren. §7: el nombre de fitxes de la nota de rendiment. `ROADMAP-PEDAGOGIC.md` F1: el cost de contingut deia «etiquetar 48 materials» per la mateixa raó | — |
| 2026-09-05 | manteniment | Preparació per al juny del 2027. `ACTUALITZACIO-ANUAL.md` passa de dos eixos a tres: hi faltava el pont `pipeline-data.js`, que no s'actualitza sol i que, si se salta, deixa el mode «El meu repartiment» congelat a la cobertura de l'any anterior. Afegits l'ordre correcte (CB → Florence → repartiment), l'ordre de lectura per a qui arribi sense context i el guió nou `valida-dades.js`, que creua tots els contractes de dades i imprimeix les xifres escrites als `.md`. Tretes de `florence-cb.html` les frases amb l'any escrit a mà: ara els cursos amb graf es dedueixen del `PAYLOAD`. El validador ja ha trobat el seu primer error real (el fil `pensament-critic` estava definit i no s'usava enlloc) | — |
| 2026-09-05 | UI | Els continguts amb seqüència de pràctica es distingien només per un to de gris (`--muted` #5f5e5a contra `--hint` #8a8a85): a la pràctica, invisible. Ara la diferència és de forma —punt ple contra anell buit— en un sol color, perquè el marcador vol dir una sola cosa i el sentit matemàtic ja l'expliquen la capçalera i la vora del tema. S'aplica a l'arbre, als resultats de cerca i a «La resta del tema». Cada tema hi afegeix un recompte «n/m», a les dues pàgines, per veure on hi ha material sense desplegar-lo. La llegenda del panell lateral deixa de repetir els colors dels sentits i explica el marcador | — |
| 2026-09-05 | funcionalitat | `repartiment.html`: camí invers del canvi anterior. Cada contingut amb fils didàctics duu a la dreta una icona que obre la seva seqüència a `florence-cb.html?c=<CURS>\|<sentit>/<tema>\|<posició>` (231 continguts). Va a la dreta i no al marge esquerre per no competir amb la icona de material del catàleg, que respon una pregunta diferent. Es pinta al mateix `buildTemaRow()` perquè les dades són locals i síncrones, a diferència de les del catàleg. S'amaga en imprimir. L'avís que segella `downloadData()` ara esmenta les dues taules indexades per posició | — |
| 2026-09-05 | funcionalitat | `florence-cb.html`: segona entrada «El meu repartiment». El professorat tria un contingut del repartiment (arbre per sentit/tema o cercador que travessa els quatre cursos) i la pàgina en munta la seqüència: tasca rica de Florence → preguntes CB (ordenades posant davant la prova que farà aquest alumnat) → material del catàleg. Les sessions d'un altre curs es proposen marcades, cosa necessària perquè Pitàgores de 2n el treballa «Quadrats inclinats», que és del graf de 3r. Fitxer de dades nou `pipeline-data.js` (48 fils didàctics, 231 dels 251 continguts mapats, els 166 ids CB fets servir). `florence-cb.html` estrena `<script src>` (`repartiment-data.js` + `pipeline-data.js`); si falten, el mode s'amaga. Estat compartible per URL (`?c=` i `?s=`) | — |
| 2026-09-05 | estructura | Preparació per a les novetats del juny del 2027. `florence-cb.html`: els dos grafs fixos passen a quatre (`d1`-`d4`, `ff1`-`ff4`); les pestanyes, els colors i la navegació es dedueixen del `PAYLOAD`, de manera que donar d'alta 1r o 4t d'ESO no demana tocar codi. El conjunt `FPDF` passa a ser el camp `pdf` de cada sessió. Afegides les gammes `--p*-1` (terra) i `--p*-4` (morat). `cb-main/index.html`: els anys del desplegable surten de `preguntes.json` i la llista escrita a mà queda només de reserva. Incorporats al repositori `retalla_cb.py` i `genera_cb_items.py`, i creada la guia `ACTUALITZACIO-ANUAL.md` | - |
| 2026-09-04 | dades | `florence-cb.html`: incorporada l'edició CB 2026. 80 referències noves repartides entre les 22 sessions (59 preguntes de les 61 disponibles); el `PAYLOAD` passa de 107 a 166 ids CB. Generades les targetes `CB158.png … CB218.png` amb `make_cb_card.py` i palatitzades (22,2 MB → 8,3 MB). Queden fora `CB213` i `CB214` (velocitat i temps de les rutes), que no encaixen amb el nucli de cap sessió. Actualitzats els recomptes a `README.md`, `MANTENIMENT.md` i `ARQUITECTURA.md` | — |
| 2026-09-04 | neteja | `florence-cb.html`: recuperats el `nucli` de cada sessió i els xips «Continua per…» (`relFor()` estava definida i no es cridava; ara conserva també el text de la relació, que es llençava). Eliminats `principal` i `addicionals[]` del `PAYLOAD` —no es renderitzaven i envellien a cada edició del banc—, el CSS orfe (`.urlbox`, `.toast`), el `<div id="toast">` invisible i les variables `--warn`, `--warn-bg`, `--green-bg`. El graf de 3r ESO ja fa servir els verds `--p*-3` en comptes de repetir els blaus de 2n. El fitxer passa de 55,4 kB a 41,4 kB | — |
